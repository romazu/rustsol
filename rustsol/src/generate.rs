...
pub fn generate_structs(nested_types: Vec<NestedType>) -> TokenStream {
    ...
    for nested_type in nested_types {
        match &nested_type {
            NestedType::Struct { label, members, number_of_bytes } => {
                ...
                let struct_size_slots = ceil_div(nested_type.size(), 32) as u64;
    ...
    let imports_definition_items: Vec<Item> = vec![
        ...
        parse_str("use alloy_primitives::{Address, I256, U256};").expect("Failed to parse"),
    ];
...
fn get_value_type_name(nested_type: &NestedType) -> TokenStream {
    let type_name = match nested_type {
        ...
        NestedType::Bytes { native_type } => {
            let native_type_tokens = proc_macro2::TokenStream::from_str(native_type).unwrap();
            quote! {#native_type_tokens}
        }
        NestedType::Mapping { .. } => {
...
fn get_nested_type(nested_type: &NestedType) -> TokenStream {
    match nested_type {
        ...
        NestedType::Bytes { native_type } => {
            let native_type_tokens = proc_macro2::TokenStream::from_str(native_type).unwrap();
            quote! { Bytes<#native_type_tokens> }
        }
